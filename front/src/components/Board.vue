<template>
    ...
    <v-row>
      <v-col class="offset-sm-0 v-col-sm-8 offset-sm-2 v-col-lg-8 offset-lg-2">
        ...
        <div class="d-flex align-center" v-if="!board.readOnly">
          ...
          <input placeholder="Type a task" class="the-add-task" v-model="task"
                 @keyup.enter="addTask($event.target.value)" @blur="addTask($event.target.value)"/>
        </div>
        <div class="my-5">
          <draggable
              :list="filteredTasks"
  ...
  <div class="the-footer">
    <div class="the-footer__content">
      <v-menu open-delay="50" location="top" class="rounded-xl" :open-on-hover="!isMobile()" :open-on-click="isMobile()"
              :close-on-content-click="false" :transition="false">
        <template v-slot:activator="{ props }">
          <v-btn v-show="isBoardHasTypeTask" value="statuses" v-bind="props" variant="text" rounded="0"
                 class="text-body-2">
            <span>Statuses</span>
      ...
      >
        ...
        <v-list class="rounded-lg" density="compact">
          ...
          >
            ...
            >{{ label.title }} {{ getLabelCount(label) }}
            </v-chip>
          </v-list-item>
          <v-list-item v-if="!board.readOnly" density="compact" value="Add" @click="showLabelsEditor()"
                       v-show="!haveLabels">Add label
          </v-list-item>
          <v-list-item v-if="!board.readOnly" density="compact" value="Edit" @click="showLabelsEditor()"
                       v-show="haveLabels">Edit
          </v-list-item>
          <v-list-item density="compact" value="Reset" @click="resetLabelFilter()" v-show="haveLabels">Reset
          </v-list-item>
        </v-list>
      </v-menu>
      <the-settings v-if="!board.readOnly" v-model:display="board.display"
                    @update:display="updateDisplay"></the-settings>
      <v-menu v-if="!board.readOnly" open-delay="50" location="top" class="rounded-lg" :open-on-hover="!isMobile()"
